...
- **SCOPE — push cannot choose one (verified live).** `sys_scope` is INERT on a Table API write: the platform sets it from the scope the REST transaction runs in (Global for `/api/now/table`) and rewrites `api_name` to match. The `apps.current_app` preference does not steer it. Consequences to tell the user about:
  - Updating an existing record is unaffected — its scope is already set.
  - CREATING a record whose artifact is scoped is REFUSED, because it would land in Global and leave the project and the instance disagreeing. `--target-scope global` is the explicit opt-in that creates it in Global anyway and reports the scope and `api_name` it actually got.
  - `--target-scope <any other scope>` is refused up front — this transport cannot deliver it. Use `update-set-package --scope <scope> --scope-id <id>`, whose payload carries `sys_scope`.
  ...
  - Local constraint: a scope-bound project cannot compile `apiName: 'global.Thing'` (TS11), so for tables carrying an `apiName` "author in a scoped project, push to Global" does not work even with `--target-scope global`. Use a Global-bound project or `update-set-package`.
...
- Other flags: `--all` (+ `--include`/`--exclude`) pushes every built record, `--full` sends every modelled field, `--no-build` skips the build, `--no-drift-check` disables the comparison. `--dry-run` still runs the (local) build, so the preview always matches your current source.
