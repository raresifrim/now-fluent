...
- **FLOWS are pushed as ONE unit.** Verified with a real SDK 4.12.2 build: a `Flow()` compiles into ONE artifact holding the flow, its trigger and step instances, and `delete_multiple` directives (`flow=<id>^sys_idNOT IN<current steps>`) that remove steps no longer in the source; the flow record always carries `active=false`/`status=draft`, because `install` activates flows afterwards (`POST api/now/wfa_fluent/activate_flows`, run as the app's scope). So selecting the flow, one of its steps, or `--all` pushes the whole artifact:
  - Every read and guard runs first (drift on the flow record — its baseline is re-taken after activation, which changes it; drift on any step with a baseline; scope rules as for a single create, including the one-time probe, which runs only after every read-only check passed). Then every record and directive is applied in document order, as the flow's scope.
  - The live flow's `active`/`status` are NEVER sent. Afterwards the flow is activated like install does if it is new or was active (`--activate` forces, `--no-activate` skips); an instance without the endpoint (it comes with the ServiceNow IDE) reports the flow as written but NOT activated, and the run fails.
  ...
  - Compressed fields (a trigger's `trigger_inputs`, a step's `values`) are gzip+base64 (`H4sI…`) in the build and are sent AS-IS. Verified live: the Table API stores and returns them unchanged (a Flow Designer trigger reads back `H4sI…`); a trigger written decompressed broke the SDK's own reader (`Corrupt data in trigger instance: incorrect header check`).
  - OPEN, seen live twice: the graph is written correctly (edits, added and removed steps all land), but `activate_flows` rejects a pushed flow with `PUBLISH_FAILED: No Trigger instance found in the flow definition` — although the trigger record exists and `install` makes the identical activation call. The cause is not known yet. Runbook phase 7 compares our flow and trigger field by field with a Flow Designer flow and counts the records attached to each; `KEEP_FLOW=1` leaves the flow on the instance to try Activate in Flow Designer by hand. Until this is solved, a pushed flow is written but NOT activated (the run fails and says so).
  ...
  - A write failing part-way leaves the flow possibly inconsistent; the message says so, and pushing again converges.
  ...
  - Runbook phases 7–8 exercise this live: create a flow in Fluent, push, activate, fire it with an incident; edit a step, add and remove one, drift, pull it back, delete it. Live so far (dev410927, `sn_sow`): the graph is written as one unit; editing a step, adding one and removing one (through `delete_multiple`) all land; the drift guard refuses a real change; pull via the online transform generates `src/fluent/generated/automation/flow/sys_hub_flow_<id>.now.ts`. Activation is the open problem above.
