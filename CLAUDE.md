...
- **SCOPE — push cannot choose one (verified live).** `sys_scope` is INERT on a Table API write: the platform sets it from the scope the REST transaction runs in (Global for `/api/now/table`) and rewrites `api_name` to match. The `apps.current_app` preference does not steer it. Consequences to tell the user about:
  ...
  - DELETES of a record in a non-Global app (the probe, a rollback, `--allow-delete`) run AS that app first and from Global only if that fails: verified live, a Global DELETE of a record inside `sn_sow` is refused with HTTP 403. The probe runs once per run even when it fails, so an undeletable probe is named once and never repeated per record; the spike's `delete-as-app` line checks this.
